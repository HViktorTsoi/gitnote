...
## 相机标定
鱼眼相机使用MEI模型(vin-mono中也用了这个模型)
鱼眼相机标定最佳实践:
1. 以0.036grid size的棋盘格为例, 最佳距离为半米左右
2. 保证要把鱼眼周围的一圈全部采集到棋盘格
3. 在0.5米的位置是成像面平行于相机, 采集整个成像平面的水平棋盘格
4. 在棋盘格的4个侧面垂直放置相机, 逐渐改变相机俯仰角, 采集不同角度的棋盘格图像
5. 最后验证: 
   1. 使用深度传感器投影, 验证环境深度与相机采集到的可见光是否一致
   2. 构造x=1, y~(-3,3), z~(-3,3)的虚拟点阵列, 观察投影的结果是否正确

标定森云时, 用pinhole模型RMSE>1, 用fisheye模型RMSE约为0.6, 这可能说明森云更适合鱼眼相机模型; 用camodocal也是这样, pinhole有0.6的误差, 用KB只有0.3的误差

用cam lidar calibration时, 森云要选择kb模型才标的准
...
# 米文时间同步系统分析
1. sync out, 在每次输出上升沿之后, 往某个文件中写入当前的时间. 因为外围设备一般为低频(100Hz), 所以这个时间一段时间内不会被改变. 传感器驱动读取这个文件之后就可以和采样时间关联起来. 注意这里米文在实现的时候, 这个时间前边是有通道的, 也就是说后续可以支持给多通道不同频率的传感器做时间同步.
2. 在使用ntp的时候, 相机时间戳异常, 推荐使用none或者gps同步
...
# 相机同步问题
验证相机是否同步, 在标定外参之前可以用kalibr, 在标定外参之后可以用vins
1. Pointgrey相机在start之后会有几十秒的预热时间, 预热的时候传输数据的速率很慢, 30Hz大概要30ms/帧; 在预热结束之后的一瞬间那一帧, 传输速率会掉到36ms, 之后所有帧的传输速率都会变快, 大概27ms/帧; 
2. 所以在预热结束的瞬间由于时间较慢, 会导致时间戳累积, 从而导致后续的时间累计误差持续错位;
3. 同时这也是因为快门速度不够快, 在30HZ的外部快门触发信号下, 曝光+相机内readout+数据传输的时间超过了触发间隔, 导致相机内部卡顿, 从而导致pc实际收到图像的时间与快门触发时间的间隔远大于两次触发的时间间隔, 这个时候pc接收到图像的还是上一帧的, 但是触发时间戳已经是下一帧的了; 这个时候在驱动里还用最近原则进行关联, 就会导致给图像的时间戳实际上是下一帧的时间, 最终导致系统错误的认为图像比imu还要早到; 
4. 这个时候图像的帧率也没法满30Hz, 为了提升帧率即使相机内部不卡顿, 需要将trigger mode改为14, 是重叠曝光模式, 这个时候帧率会到30Hz, 但是同步时间戳关联的问题还是存在;
