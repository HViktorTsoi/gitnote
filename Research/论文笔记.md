...
3. 用4个不同的kernel size，在最后的两个维度上做池化，得到4个不同的HxWxC的correction矩阵，这样可以获得某一帧在另一帧上不同尺度的相关性信息；
4. 求光流是用迭代的方式进行的，对于每一次迭代出来的光流(初始光流设置为0)，将第一帧的像素x用这个光流warp到后一帧上得到坐标x'，然后取出x'周围所有邻居的坐标，可以把他们看作是后一帧上的像素 [公式] 在 [公式] 上的潜在位置；
如何在correction矩阵上查询：
	1. 对于第一帧的某个像素，首先用上一次迭代的光流