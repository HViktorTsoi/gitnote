...
这里就是使用了2层mlp来提取点云的特征，与pointnet有区别的是没有使用T-Net, 这样PQ就是不对齐的，当二者有旋转的时候，会有完全不同的xyz数值，因此通过这样的encoder会产生完全不同的特征
...
为什么要用sinkhorn, 因为点云/图像配准是一个多对多匹配的问题, 如果简单的选择行/列的最大值, 很容易产生误匹配,而这里就适合考虑全局的匹配方法, 比如Mij点虽然是在这一行/列中匹配度最高的一个元素, 但是在单点匹配的假设下, 如果认为Mij是一个匹配, 那么必然有其他点和i/j无法匹配, 如果在这种情况下造成了整体匹配系数的下降, 那么说明Mij并不是一个好的匹配 
