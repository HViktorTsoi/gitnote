...
- 解答： 在BA之后，整个voxel的结构不会被更新，每一棵octree的结构也不会被更新。这样就可能有一种情况：某个voxel在BA之后可能位置有较大的的变化，其所在的新坐标在计算hash之后已经对应不到这个voxel了，这样的话当有新的点到达这个新位置之后，在hash表中查找的时候，算出来的位置也是对应不到这个voxel的，那么只能在这个新的voxel位置上新建一课octree；这样一来原本属于同一颗树的两个