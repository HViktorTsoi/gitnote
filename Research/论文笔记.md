...
所以对于lidar BA来说，就是同时优化每一帧位姿T，每个plane特征的位置(q,n)，每条edge特征的位置(q, n)
(因此就需要显式的把特征描述计算出来，这根loam系列是有区别的)
...
4. 这样下来每个voxel对应一个特征描述子；同时构建地图的时候有每一帧位姿信息，这样就可以进行BA了；
5. 在实现的时候，维护两个voxel map，一个plane，一个edge；首先对点云划分多个voxel，每个voxel再对应一个octree（这样可以减少树的深度），
6. 如果voxel中的点过多，就对某些点做平均，用平均点代替之前的点做BA，这样可以