...
3. 这里简单提一下MCL：
	...
	4. 3里就是下边这个公式描述的，积分那项是所有粒子都进行运动估计，观测那项是在当前的位置，观测到当前这一帧LiDAR输入，有多大可能性（其实就是overlapnet的输出，overlapnet保存的就是LiDAR scan随空间的概率分布，这里的空间是通过virtual scan来表示的，因为每个virtual都独立的代表了一个空间位置。所以P(z|x)就相当于overlapnet(LiDAR scan|virtaul scan)）；
