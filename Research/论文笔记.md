...
3. laser里程计。这部分是使用帧间特征匹配来求解两帧点云的相对位姿。(last表示上一帧)
	...
	2. 对于角点特征，使用点-线ICP，具体方法是，对于当前帧的每一个点，在last sharp中找他的邻居点，然后通过pca判断邻居的形状，只有邻居构成一条线(协方差的最大特征值远大于其他两个)才用这个特征点参与计算；
	3. 角点对loss的贡献就是角点到邻域线的距离；
	4. 对于平面特征，使用点-面ICP，具体方法是，对于当前帧的每一个点，在last中找他的邻居点
