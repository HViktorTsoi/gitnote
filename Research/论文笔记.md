...
4. 之前这些步骤都是用共享的参数来提单个点云特征和构建拓扑图的过程, 下边重点是这个, 对两帧点云的匹配进行特征关联这部分, 也就是图匹配的过程
   ...
   2. 然后用sinkhorn算法(这是一个求解最优分配的算法), 把相似度矩阵转换为匹配矩阵
   3. 最后, 为了把两个图的信息再次融合起来, 进行一个图之间的卷积, 这个过程与通常的图卷积是类似的, 只不过通常的图卷积是在自己的邻居节点上做特征的聚合, 而这个图间的卷积,使用上一步得到的匹配矩阵来作为邻接矩阵, 也就是说每个节点聚合的是匹配节点的特征
