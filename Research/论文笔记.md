...
1. 输入是N个keypoint-correspondence的坐标，每个correspondence 是$(x_i, y_i)\in R^6$ (这样是不是只使用了匹配的信息，而没有使用原始点云的任何信息？)
...
the overall correspondence mapping (one-to-one or one-tomany).
...
∗
删除掉多余的correspondence，但是没有修正correspondence