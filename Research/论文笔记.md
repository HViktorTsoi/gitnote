...
具体做法是，对于双生网络的两个leg输出的HxWxC的feature volume，先将其展开成HWxC的一维feature，然后对其中一个在横向复制HW倍，另一个在纵向复制HW倍(变成两个HWxHWxC的feature volume)，然后对这两个进行逐元素

correlation-head：求解两个点云之间的yaw角偏移：
对于双生网络的两个leg输出的feature volume，将其中一个复制1倍之后，用另一个做卷积核去卷他，以这样的方式对二者做互相关，然后相关的结果是一个360维的向量，对应360个度，激活值最大的那个位置对应的向量就是二者的yaw偏移。
