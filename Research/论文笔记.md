...
4. 做完scan-map matching之后，更新voxel map：首先看特征点落在哪个voxel里，更新其对应的octree；如果不在已有voxel里，就构建一棵新的octree；octree中存储了每一个特征点，以及特征点对应滑窗中的第几帧，
5. 当voxel凑齐滑动窗口大小之后，就开始BA；BA完成之后用新的位姿和新的特征点坐标更新voxel的n和q；
