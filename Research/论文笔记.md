...
# Vid2vid 代码实现
## GPU split方式
0. 如果n_gpus_gen设置成-1，就在parse参数的时候自动把n_gpus_gen改变为gpu_ids的长度，即使用所有gpu;
1. 如果 n_gpus_gen 和 可用的gpu_id 相同，那么就把batch填充到gpu数量的整数倍，然后把数据平均并行到所有gpu上(注意这里至少要保证batch>gpu数量)；
2. 如果 n_gpus_gen 小于 可用的gpu_id:
	1. 如果batch_size为1，就把generator放在第0块gpu，discriminator和flow并行到所有gpu上；