...
3. 重要：和loam很大不同，在map中搜索当前帧的某个特征点可能的匹配邻域时，不是在整个map构造的kdtree上搜索的，而是直接先找当前这个特征点在哪个voxel里；由于在构造voxel的时候，就计算了每个voxel对应的n和q，因此直接计算当前点到voxel所在平面/直线的距离作为残差(而不是像loam一样先拟合邻域构成的面/线，再计算残差)；
